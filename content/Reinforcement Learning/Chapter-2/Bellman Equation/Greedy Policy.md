...
---
## Greedy Policy
`Greedy policy` is the mapping that selects an action by choosing the maximizer of the [[Quality Function|action-value function]].

- For $Q \in \mathcal{B}(\mathcal{X} \times \mathcal{A})$, the greedy policy $\pi_{g}: \mathcal{X} \times \mathcal{B}(\mathcal{X} \times \mathcal{A}) \to \mathcal{A}$ is
...
$$
\pi_{g}(s; V) = \arg\max_{a \in \mathcal{A}}
\left\{  r(s,a) + \gamma \int 
\mathcal{P}(ds' \mid s,a) \ V(s')  \right\}
$$
- As proven [[Bellman Equation for Optimal Quality Functions|here]], $\pi_{g}(V^{*}) = \pi_{g}(Q^{*}) = \pi^{*}$

---
### Intution behind Greedy Policy
- It chooses the action only based on the `local information`.
- It only looks one step ahead $(\text{for V})$ and no-step ahead $\text{(for Q)}$.
  This is called `myopic action selection` mechanism.
- This is because the optimal value functions encode the information about the future. 
  So, we do not need to explicitly consider all possible futures.
